...
function ENT:Initialize()
	...
	self:InitPhysics()
	self.delayRemove = CurTime() +3.8
	ParticleEffectAttach("archvile_plasma",PATTACH_ABSORIGIN_FOLLOW,self,0)
	self.m_speed = self.m_speed || 200
	self.m_radius = self.m_radius || 50
	self.m_dmg = self.m_dmg || 18
	
	local light = ents.Create("light_dynamic")
	light:SetKeyValue("_light","255 50 0 200")
	light:SetKeyValue("brightness","0")
	light:SetKeyValue("distance","1600")
